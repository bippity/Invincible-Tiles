...
namespace InvincibleTiles
{
    ...
    public class InvincibleTiles : TerrariaPlugin
    {
        ...
        {
            get { return "Invincible Tiles-Reversed"; }
        }
        ...
        {
            get { return "Makes all tiles indestructable"; }
        }
        ...
        private void AddTile(CommandArgs args)
        {
            ...
            {
                args.Player.SendMessage(String.Format("Successfully blacklisted {0}",id), Color.Red);
                blacklistedTiles.Add(id);
            }
        }
        ...
        private void DelTile(CommandArgs args)
        {
            ...
            if (db.Query(query, id, 0) != 1)
            {
                ...
                args.Player.SendMessage(String.Format("Removing from the database has failed!  Are you sure {0} is unbanned?", id), Color.Red);
            }
            else
            {
                args.Player.SendMessage(String.Format("Successfully banned {0}", id), Color.Green);
